...
export async function searchSchoolsByName(query: string): Promise<SchoolCandidate[]> {
  ...
  try {
    ...
    return rows.map((row: Record<string, string>) => {
      const kind: string = row.SCHUL_KND_SC_NM ?? "";
      const schoolType: SchoolCandidate["schoolType"] =
        kind.includes("초등") ? "초등학교"
        : kind.includes("중학") ? "중학교"
        : kind.includes("고등") ? "고등학교"
        : "기타";
      return {
        name: row.SCHUL_NM,
...
export async function fetchMealByDate(
  ...
  try {
    ...
    if (rows.length === 0) {
      return { menuText: "오늘 급식 정보가 없습니다.", source: "neis" };
    }
